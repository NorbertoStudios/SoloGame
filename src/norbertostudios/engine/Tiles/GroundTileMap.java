...
public class GroundTileMap {
    ...
    public static ArrayList<Rectangle> groundRectangleArrayList;

    // Constructor #2
    public GroundTileMap(int[][] existingMap) {


        groundRectangleArrayList = new ArrayList<>();
        ...
        for (int y = 0; y < map.length; y++) {
            for (int x = 0; x < map[y].length; x++) {
                ...
                if (index != 0) {
                    groundRectangleArrayList.add(new Rectangle(x * 64 - MainCamera.x, y * 64 - MainCamera.y, 128, 128));
        ...
        for (int y = yStart; y < yEnd; y++) {
            for (int x = xStart; x < xEnd; x++) {
                ...
                if (index != 0) {
                    ...
                    tile = new Tile(x, y);
                    tile.draw(g);
                    g.drawImage(Assets.groundMapHashMap.get(index - 1),
                            x * 64 - MainCamera.x,
                            y * 64 - MainCamera.y,
                            null);
